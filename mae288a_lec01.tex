...
Optimal control is concerned with reaching some specified goal that is given in advance. There is some flexibility in selecting the goal that is related to proposing a goal that is solvable and that can be considered correct for the operating regime -- the goal must align with the desired outcome for the system. Optimal control is concerned with several types of dynamic systems:

\begin{itemize}
  \item Continuous time deterministic
  \item Discrete time deterministic
  \item Continuous time stochastic
  \item Discrete time stochastic in continuous space
  \item Discrete time stochastic in discrete time
  \item Games
\end{itemize}
...
\begin{align*}
\dot\zeta_t = f(\zeta_t,u_t) \textnormal{, where } \zeta_0 = x \in \mathbb{R}^n \textnormal{, }
\end{align*}
The state is represented as $\zeta(t)$ and the control input is represented by $u(t)$.

Discrete time deterministic dynamics are generally known as ``classical optimal control'' and given by
\begin{align*}
