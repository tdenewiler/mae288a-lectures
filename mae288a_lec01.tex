...
``$P(A)$'' $= P\left(X^{-1}(A)\right) = \int_A f(x)dx$. $\lozenge$
...
Since $x$ is a R.V. and $E(x)$ is a scalar the variance is a scalar as well. $\lozenge$

\section{Covariance}
\textit{Example:} Using (\ref{eq:probEg}) in $\mathbb{R}^n$ we have $$C_{ij} \doteq E[(x_i - E(x_i))(x_j - E(x_j)].$$ Note that $E(x_i) = \lbrace E(x) \rbrace_i$. $\lozenge$
...
\end{align*}
and by symmetry $E[x_2] = \frac{1}{2}$. This makes sense because we expect the value to be right in the middle of the area described by the function. $\lozenge$
\begin{figure}[ht!]
	\centering
