...
Let $\left(\begin{array}{c}x \\y\end{array}\right) \sim \mathcal{N}\left(\left(\begin{array}{c}0\\0\end{array}\right), C\right)$ and $C^{-1} = \left[\begin{array}{c c} \rho_{xx} & \rho_{xy} \\ \rho_{xy} & \rho_{yy} \end{array}\right]$. Also, let $\hat{k}=e^{-\frac{1}{2}\rho_{yy}d^2}$, $\bar{k}=\frac{1}{2\pi|C|^{1/2}}$ $\sigma^2=\frac{1}{\rho_{xx}}$, $m=-\frac{\rho_{xy}}{\rho_{xx}}d$ and $\tilde{k}=\frac{1}{\sqrt{2\pi}\sigma}$. Then,
\begin{align*}
...
&= P(f(\xi_t,w_t)\in A|\xi_t=x_t, \xi_{t-1}=x_{t-1}, \ldots, \xi_0=x_0) \\
&= P(f(x_t,w_t)\in A|f(x_{t-1},w_t)=x_{t-1}, f(x_{t-2},w_{t-2})=x_{t-2},\ldots,f(x_0,w_0)=x_1,\xi_0=x_0) \\
&\text{Let }g_x(w) \doteq f(x,w) \\
